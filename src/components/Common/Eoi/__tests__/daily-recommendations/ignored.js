...
describe('Eoi dailyRecommendations connectionStatus: ignored', () => {
  ...
  factory.profiles.forEach(profile => {
    describe(profile.name, () => {
      ...
      describe('type: dailyRecommendations', () => {
        const props = eoiProps('dailyRecommendations', { flags: { connectionStatus: 'ignored' } });
        it('should render ignored message', () => {
          const eoi = mount(<Eoi {...props} />);
          expect(eoi.text()).toContain(`${props.profile.name}'s Profile has been Ignored`);
        });
      });
    });
  });
