...
const facet = (name, payload, criteria = []) => {
  ...
  return {
    ...
    isMulti: !isRadio[name],
    isNested: false,
    options: ((name !== 'geo_location' && [all]) || []).concat(
      Object.keys(payload)
        .sort(
          (a, b) => (name === 'geo_location' ? 0 : t(name, a) < t(name, b) ? -1 : t(name, a) > t(name, b) ? 1 : payload[a] - payload[b]),
        )
        .map((k, i) => [k, i + 1])
        .sort(
          ([a, aOrder], [b, bOrder]) =>
...
export default (payload = {}, criteria = {}) =>
  Object.keys(payload)
    ...
    .filter(f => (f.id === 'geo_location' && f.options.length > 0) || f.options.length > 1)
