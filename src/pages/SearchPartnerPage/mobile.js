...
class SearchPartnerPage extends React.Component {
  ...
  onAction(uid, type, ...args) {
    this.props.doProfileAction('preferredSearch', uid, type, ...args);
  }
  ...
  render() {
    ...
    console.log('render SearchPartnerPage');
    return (
...
const selector = (state, { location }) => {
  const { chat, preferredSearch, successStories, profiles, session, view } = state;
