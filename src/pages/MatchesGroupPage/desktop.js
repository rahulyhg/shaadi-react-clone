...
class MatchesGroupPage extends React.PureComponent {
  ...
  renderSections(searchType, results) {
    ...
    const daTracking = src => (event, args = {}) => {
      const doDa = this.props.location.pathname.indexOf('profile/viewed') === -1;
      doDa && this.props.doDaTracking(src, event, args.uid, args);
    };
    if ((results.remark && results.remark.messageCode) || results.items.length) {
      return (
        <SearchGroups
          ...
          daTracking={daTracking(searchType)}
        />
      );
    }
    return null;
