...
const searchHeaderMsg = {
  ...
  broader: { default: 'Profiles that are based on your broadened Partner Preferences' },
  reverse: { default: 'Reverse Matches: Members whose Partner Preferences match your Profile' },
  '2-way': { default: "2-Way Matches: Profiles where both of you match each other's Partner Preferences" },
  discovery_newly_joined: { default: 'Here are some interesting Matches who joined in the last 14 days' },
  discovery_newly_joined_2way: { default: 'Here are some interesting Matches who joined in the last 14 days' },
  discovery_premium: { default: 'Premium Members that match your Partner Preferences' },
  discovery_premium_2way: { default: 'Premium Members that match your Partner Preferences' },
  discovery_recent_visitors: { default: 'These Matches have visited your profile in the last 30 days' },
  discovery_recent_visitors_two_way: { default: 'These Matches have visited your profile in the last 30 days' },
...
class SearchPage extends React.PureComponent {
  constructor(props) {
    ...
    this.discoveryCntBox = this.discoveryCntBoxA.bind(this);
    this.changeObjectProps = this.changeObjectProps.bind(this);
  ...
  componentWillReceiveProps(props) {
    ...
    if (
      props.settings &&
      props.settings.experiments &&
      (!this.props.settings.experiments || JSON.stringify(this.props.settings.experiments) !== JSON.stringify(props.settings.experiments))
    ) {
      const { experiments } = props.settings;
      const nearMe = (experiments && experiments.near_me && experiments.near_me.bucket) || 'A';
      if (nearMe === 'B') {
        this.changeObjectProps('near_me');
      }
    }
  ...
  onAction(uid, type, ...args) {
    this.props.doProfileAction('otherSearch', uid, type, ...args);
  }
  ...
  changeObjectProps(campaign) {
    switch (campaign) {
      case 'near_me': {
        this.discoveryCntBox = this.discoveryCntBoxB;
        searchHeaderMsg.discovery_premium.default = 'Recently upgraded Premium members';
        searchHeaderMsg.discovery_premium_2way.default = 'Recently upgraded Premium members';
        searchHeaderMsg.discovery_recent_visitors.default = 'Members who recently visited your Profile';
        searchHeaderMsg.discovery_recent_visitors_two_way.default = 'Members who recently visited your Profile';
        searchHeaderMsg.broader.default = 'Members who match many of your Preferences';
        searchHeaderMsg.reverse.default = 'Members looking for matches like you';
        searchHeaderMsg.discovery_newly_joined.default = 'Recently Joined members that match many of your Preferences';
        searchHeaderMsg.discovery_newly_joined_2way.default = 'Recently Joined members that match many of your Preferences';
      }
      // no default
    }
  }
  ...
  discoveryCntBoxA(searchType) {
    if (
      ![
        'discovery_newly_joined',
        'discovery_premium',
        'discovery_recent_visitors',
        'discovery_newly_joined_2way',
    ...
    const recentlyJoinedArr = ['discovery_newly_joined', 'discovery_newly_joined_2way'];
    ...
    return (
      <ss.DiscoverTabWrap>
        <ss.DiscoverTabLink to={'/search/discovery/recently-joined'} isActive={recentlyJoinedArr.includes(this.props.searchType)}>
          Recently Joined{' '}
          {recentlyJoinedArr.includes(this.props.searchType)
            ? this.props.enableLinkCount > 0 && `(${this.props.enableLinkCount})`
            : this.props.allLinkCount.discovery_newly_joined > 0 && `(${this.props.allLinkCount.discovery_newly_joined})`}
        </ss.DiscoverTabLink>
        <ss.DiscoverTabLink to={'/search/discovery/premium'} isActive={premiumArr.includes(this.props.searchType)}>
          Premium Matches{' '}
          {premiumArr.includes(this.props.searchType)
            ? this.props.enableLinkCount > 0 && `(${this.props.enableLinkCount})`
            : this.props.allLinkCount.discovery_premium > 0 && `(${this.props.allLinkCount.discovery_premium})`}
        </ss.DiscoverTabLink>
        <ss.DiscoverTabLink to={'/search/discovery/recent-visitors'} isActive={recentlyVisitorArr.includes(this.props.searchType)}>
          Recent Visitors{' '}
          {recentlyVisitorArr.includes(this.props.searchType)
            ? this.props.enableLinkCount > 0 && `(${this.props.enableLinkCount})`
            : this.props.allLinkCount.discovery_recent_visitors > 0 && `(${this.props.allLinkCount.discovery_recent_visitors})`}
        </ss.DiscoverTabLink>
      </ss.DiscoverTabWrap>
    );
  }
  discoveryCntBoxB(searchType) {
    if (
      ![
        'discovery_premium',
        'discovery_recent_visitors',
      ...
      <s.SearchPageWrapper topSpace={this.props.topSpace} isChatOpen={this.props.isChatOpen} windowWidth={this.props.windowWidth}>
        <s.SearchPartnerPage>
          ...
          <s.SearchResults>
            ...
            <s.NoDataWrapper isVisible={this.props.flash}>
...
SearchPage.propTypes = {
  ...
  enableLinkCount: PropTypes.number.isRequired,
  allLinkCount: PropTypes.shape({
    matches: PropTypes.number,
    recommendations: PropTypes.number,
    discovery_newly_joined: PropTypes.number,
    discovery_premium: PropTypes.number,
    discovery_recent_visitors: PropTypes.number,
    discovery_newly_joined_2way: PropTypes.number,
    discovery_premium_2way: PropTypes.number,
    discovery_recent_visitors_two_way: PropTypes.number,
  }).isRequired,
...
const selector = (state, { location }) => {
  const { chat, otherSearch, successStories, profiles, session, view } = state;
  ...
  return {
    ...
    enableLinkCount: searchType.count,
