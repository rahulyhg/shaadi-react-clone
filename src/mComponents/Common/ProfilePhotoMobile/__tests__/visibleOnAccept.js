...
describe('ProfilePhotoMobile albumStatus: visibleOnAccept', () => {
  const onAction = jest.fn();
  const profilePhototProps = (albumStatus, uid) => ({ ...fixtureProps, albumStatus, uid, onAction });
  const mount = createMount();
  let profilePhotoMobile;
  beforeAll(() => {
    const props = profilePhototProps('visibleOnAccept', 'sample-uid');
  ...
  // const buttonsSet = [{ index: 0, type: 'request photo password', text: 'Request Photo Password', onClickArgs: ['request_password'] }];
  ...
  it('should have correct message', () => {
    expect(profilePhotoMobile.text()).toContain('Visible on Accept');
  });
