...
describe('ProfilePhotoMobile albumStatus: passwordRequested', () => {
  const onAction = jest.fn();
  const profilePhototProps = (albumStatus, uid) => ({ ...fixtureProps, albumStatus, uid, onAction });
  const mount = createMount();
  let profilePhotoMobile;
  beforeAll(() => {
    const props = profilePhototProps('passwordRequested', 'sample-uid');
  ...
  it('should have correct message', () => {
    expect(profilePhotoMobile.text()).toContain('Password Requested');
  });
