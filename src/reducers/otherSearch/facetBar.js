...
const recentlyViewed = (state = inititalRecentlyViewedState, action) => {
  switch (action.type) {
    case types.OTHERSEARCH_FACET_UPDATE: {
  ...
  switch (action.type) {
    ...
    case types.OTHER_SEARCH_SUCCESS: {
      const newState = {
        ...
        options: state.options.map(o => ({
          ...o,
          isSelected: action.payload.searchTypeDetail.setMatchesFacetValue === o.value,
        })),
      };
      return JSON.stringify(newState) === JSON.stringify(state) ? state : newState;
    }
...
export default function(state = initialState, action) {
  switch (action.type) {
    ...
    case types.OTHER_SEARCH_SUCCESS: {
      const { searchTypeDetail } = action.payload;
      let newState = {};
      if (!searchTypeDetail.showRecentViewFacet && !searchTypeDetail.showMatchesFacet) {
        newState = action.payload.facets.map(f => facet(undefined, { ...action, payload: f })).filter(f => f.options && f.options.length);
        return JSON.stringify(newState) === JSON.stringify(state) ? state : newState;
      }
      if (searchTypeDetail.showRecentViewFacet && searchTypeDetail.showMatchesFacet) {
        newState = [recentlyViewed(state[0], action), matches(state[1], action)]
          .concat(action.payload.facets.map(f => facet(undefined, { ...action, payload: f })))
          .filter(f => f.options && f.options.length);
        return JSON.stringify(newState) === JSON.stringify(state) ? state : newState;
      }
      if (searchTypeDetail.showRecentViewFacet && !searchTypeDetail.showMatchesFacet) {
        newState = [recentlyViewed(initialState[0], action), {}]
          .concat(action.payload.facets.map(f => facet(undefined, { ...action, payload: f })))
          .filter(f => f.options && f.options.length);
        return JSON.stringify(newState) === JSON.stringify(state) ? state : newState;
      }
