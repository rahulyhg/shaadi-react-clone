...
export default function(state = initialState, action) {
  switch (action.type) {
    ...
    case types.INBOX_DATA_SUCCESS: {
      ...
      if (!['connect_pending', 'connect_filtered'].includes(listType)) {
        return initialState;
      }
      const { evtRef, results_id, permalink } = action.payload.featuredItems.meta || {};
      return {
        ...state,
        results_id,
        evt: evtRef,
        ...
        displayed_request_id: action.payload.featuredItems.meta && action.payload.featuredItems.meta.request_id,
        permalink,
        items: action.payload.featuredItems.items || state.items,
      };
    }
