...
const requestsAcceptedInitialState = {
  key: 'requests',
  label: 'Requests',
  path: '/inbox/accepted/requests',
...
const callSmsInitialState = {
  key: 'callSms',
  label: 'Contacts',
  path: '/inbox/contact-summary',
  url: '/inbox/contact-summary',
  refs: ['inbox-contactsavailed'],
  isExternal: false,
  isActive: false,
  count: 0,
};

const callSms = (state = callSmsInitialState, action) => {
  switch (action.type) {
    default:
      return commmonSubReducer(state, action);
  }
...
const nav = (state, action) => {
  const newState = [
    ...
    sent(state[3], action),
    callSms(state[4], action),
    deleted(state[5], action),
  ];
  ...
  switch (type) {
    ...
    case types.COUNTS_SUCCESS: {
      ...
      const defaultStateMap = {
        invitations_new: invitationsInitialState,
        accepted_new: acceptedInitialState,
        newRequestsPending: requestsInitialState,
        newRequestsAccepted: requestsAcceptedInitialState,
      };
      const LandinglistType = Object.keys(defaultStateMap).find(listType => payload.counts[listType] > 0);
      const newNavState = LandinglistType ? defaultStateMap[LandinglistType] : invitationsInitialState;

      return {
        ...state,
        url: newNavState.url,
        otherPaths: newNavState.otherPaths,
