...
module.exports =
{
  '@tags': ['regression','email_pass_profile'],
  ...
  'Validate correct email message' : function(browser) 
  {
    const reg_basic_details = new Reg_Basic_Details(browser);

    let emails = test_data["valid_email"]["emails"];
    ...
    for (const email of emails) 
    {
      ...
      browser
      .verify.containsText('//*[@id="err_text_invalid_email"]', "",email);
    }
  },
  ...
  'validate password for spaces ' : function(browser) 
  {
    const reg_basic_details = new Reg_Basic_Details(browser);

    reg_basic_details.setPassword(test_data["password_error"]["spaces_password"]);
    reg_basic_details.clickNext();
  ...
  'test validate profile for' : function(browser)
  {
    let postedBy = ["Self","Son","Daughter","Brother","Sister","Friend","Relative"];

    const reg_basic_details = new Reg_Basic_Details(browser);

    for(let item of postedBy)
    {
      reg_basic_details.setCreatedFor(item);
      
      if(item == "Self" || item == "Friend" || item == "Relative")
        browser.verify.visible("//*[@id='layer_gender-Male']", item + " is selected");
      else
        browser.waitForElementNotVisible("//*[@id='layer_gender-Male']", 50, false, ()=>{}, item + " is selected");
    }
  }
  ,
